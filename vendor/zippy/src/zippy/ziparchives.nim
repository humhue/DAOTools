...
type
  StreamEntryRecord = object
    path: string
    crc: uint32
    size: uint32
    offset: uint32 # The exact physical byte where this file's header starts

  ZipStreamWriter* = ref object
    stream: FileStream
    currentOffset: uint32
    records: seq[StreamEntryRecord]

proc openZipStream*(path: string): ZipStreamWriter =
  new(result)
  result.stream = newFileStream(path, fmWrite)
  if result.stream == nil: quit("Failed to open output stream: " & path)
  result.currentOffset = 0
  result.records = newSeq[StreamEntryRecord]()

proc addEntry*(writer: ZipStreamWriter, path: string, data: sink string) =
  let crc = crc32(data)
  let size = data.len.uint32
  let pathLen = path.len.uint16
  
  # 1. Log the metadata for the Central Directory BEFORE we advance the offset
  writer.records.add(StreamEntryRecord(
    path: path, crc: crc, size: size, offset: writer.currentOffset
  ))
  
  # 2. Write Local File Header (Signature: 0x04034b50)
  writer.stream.write(0x04034b50'u32) 
  writer.stream.write(20'u16)         # Version needed to extract (2.0)
  writer.stream.write(0'u16)          # General purpose bit flag
  writer.stream.write(0'u16)          # Compression method (0 = Store)
  writer.stream.write(0'u32)          # Time/Date (Dummy zeroed out)
  writer.stream.write(crc)            # CRC-32
  writer.stream.write(size)           # Compressed size
  writer.stream.write(size)           # Uncompressed size
  writer.stream.write(pathLen)        # File name length
  writer.stream.write(0'u16)          # Extra field length
  writer.stream.write(path)           # Filename string
  
  # 3. Dump raw data to SSD
  if size > 0:
    writer.stream.write(data)
    
  # 4. Advance our physical byte tracker (30 bytes for header + path length + data size)
  writer.currentOffset += 30'u32 + pathLen.uint32 + size

proc close*(writer: ZipStreamWriter) =
  let centralStartOffset = writer.currentOffset
  var centralSize = 0'u32

  # 1. Write the Central Directory Headers
  for rec in writer.records:
    let pathLen = rec.path.len.uint16

    # Signature: 0x02014b50
    writer.stream.write(0x02014b50'u32) 
    writer.stream.write(20'u16)         # Version made by
    writer.stream.write(20'u16)         # Version needed to extract
    writer.stream.write(0'u16)          # General purpose bit flag
    writer.stream.write(0'u16)          # Compression method (0 = Store)
    writer.stream.write(0'u32)          # Last mod file time/date
    writer.stream.write(rec.crc)        # CRC-32
    writer.stream.write(rec.size)       # Compressed size
    writer.stream.write(rec.size)       # Uncompressed size
    writer.stream.write(pathLen)        # File name length
    writer.stream.write(0'u16)          # Extra field length
    writer.stream.write(0'u16)          # File comment length
    writer.stream.write(0'u16)          # Disk number start
    writer.stream.write(0'u16)          # Internal file attributes
    writer.stream.write(0'u32)          # External file attributes
    writer.stream.write(rec.offset)     # Relative offset of local file header!
    writer.stream.write(rec.path)       # File name string

    # 46 bytes for central header + path length
    centralSize += 46'u32 + pathLen.uint32 

  # 2. Write End of Central Directory (EOCD) Record
  # Signature: 0x06054b50
  writer.stream.write(0x06054b50'u32)             
  writer.stream.write(0'u16)                      # Number of this disk
  writer.stream.write(0'u16)                      # Disk where central directory starts
  writer.stream.write(writer.records.len.uint16)  # Total number of central directory records on this disk
  writer.stream.write(writer.records.len.uint16)  # Total number of central directory records
  writer.stream.write(centralSize)                # Size of central directory
  writer.stream.write(centralStartOffset)         # Offset of start of central directory
  writer.stream.write(0'u16)                      # ZIP file comment length

  # Safely flush and close the file
  writer.stream.close()